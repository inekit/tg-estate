...
class ConfineChecker {
  ...
  passedConfines(text, confineNames, stepName) {
    ...
    for (let name of confineNames) {
      if (typeof name === "function") {
        if (!name(text)) return;
      } else if (this.confines[name] && !this.confines[name](text)) return;
    }
  ...
  confines = {
    ...
    address(text) {
      return /^([а-яА-ЯЁё]{2,255})\s([0-9\/\.]{1,5})(\s([\wа-яА-ЯЁё0-9]{1,2}))?$/g.test(
        text
      );
    },
    phone(text) {
      return /^((8|\+7)[\- ]?)?(\(?\d{3,5}\)?[\- ]?)?[\d\- ]{7,10}$/g.test(
        text
      );
    },
    ...
    less100(text) {
      if (parseInt(text) <= 100) return true;
    },
